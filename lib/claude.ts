...
/**
 * Stream Claude tokens to `onToken` as they arrive, then return the fully
 * post-processed BreakdownResult. Used by the streaming breakdown route so
 * the user sees text appear while the server processes the request.
 *
 * Applies identical post-processing to generateBreakdown(). If the fragility
 * dedup retry fires (rare), falls back to a non-streaming second call via
 * generateBreakdown() — the user has already seen all streamed tokens by then.
 */
export async function streamNBABreakdown(
  data: GameDetailData,
  ...
  // Build system prompt — same logic as generateBreakdown()
  const now = new Date();
  const month = now.getMonth() + 1;
  const year  = now.getFullYear();
  const day   = now.getDate();
  ...
  // Confirmed-OUT names for structural dedup — same as generateBreakdown()
  const confirmedOutNames = [...data.injuries.homeInjuries, ...data.injuries.awayInjuries]
    .filter((p) => p.status === "Out")
    .map((p) => p.playerName);
  ...
  // Stream Claude, accumulate full text
  let fullText = "";
  ...
  // Parse — identical to callOnce() in generateBreakdown()
  const json = fullText.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "").trim();
  let p: BreakdownResult;
  try {
    p = JSON.parse(json) as BreakdownResult;
  } catch {
  ...
  // Post-process — identical to callOnce()
  if (!p.decisionLens) {
    p.decisionLens = CLOSING_LINE;
  } else if (!p.decisionLens.includes(CLOSING_LINE)) {
    p.decisionLens = p.decisionLens.trimEnd() + " " + CLOSING_LINE;
  ...
  // Fragility dedup
  let dedup = deduplicateFragilityCheck(p.fragilityCheck, confirmedOutNames, p.gameShape);
  if (dedup.removedCount > 0) {
    console.warn(`[breakdown:NBA] streaming fragility dedup: removed ${dedup.removedCount} —`, dedup.log);
    p.fragilityCheck = dedup.items;
  }

  // Retry: if dedup left fewer than 2 points, fall back to a non-streaming second call.
  // The user has already seen all streamed tokens; the structured view just shows
  // the retry result.
  if (p.fragilityCheck.length < 2 && dedup.removedCount > 0) {
    console.warn("[breakdown:NBA] streaming dedup retry — falling back to non-streaming second call");
    return generateBreakdown(data);
  }
